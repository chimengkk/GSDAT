...
from .swin2sr_arch import SwinTransformerBlock
...
class FAB(nn.Module):
    ...
    def __init__(self,
        ...
        # self.c1_r = conv_layer(dim, dim, 3)
        # self.esa2 = ESA(dim, nn.Conv2d)

    def forward(self, x, x_size, rpi_sa, attn_mask):
        # b, l, c = x.shape
        # # print(x.shape)
        # x = x.view(b, Height, Width, c).permute(0, 3, 1, 2)
        # x = self.esa2(x)
...
class AttenBlocks(nn.Module):
    ...
    def __init__(self,
        ...
        self.use_swinv2 = use_swinv2  # 记录开关状态
        self.esa_pos = esa_pos  # 记录ESA位置状态
        ...
        for i in range(depth):
            # 【逻辑1：前置 ESA】
            if self.use_esa and i % 2 == 0 and self.esa_pos == 'front':
                blk.append(ESA(dim, nn.Conv2d))

            # 【逻辑2：选择使用 SwinV2 还是 原始SwinV1】
            if self.use_swinv2:
                # 使用 SwinV2 (你导入的模块)
                blk.append(
                ...
                # 退化为 SwinV1 (调用你文件里原有的 FAB 模块)
                blk.append(
                    FAB(dim=dim, input_resolution=input_resolution,
                        num_heads=num_heads, window_size=window_size,
                        shift_size=0 if (i % 2 == 0) else window_size // 2,
                        mlp_ratio=mlp_ratio,
            ...
            # 【逻辑3：后置 ESA】
            if self.use_esa and i % 2 == 0 and self.esa_pos == 'back':
                blk.append(ESA(dim, nn.Conv2d))
...
class GSDATNet(nn.Module):
    ...
    def __init__(self,
        ...
        esa_count = sum(1 for m in self.modules() if m.__class__.__name__ == 'ESA')
        gab_count = sum(1 for m in self.modules() if m.__class__.__name__ == 'GAB')
        total_params = sum(p.numel() for p in self.parameters())

        print(f'[DEBUG] use_esa={use_esa}, use_gab={use_gab}, esa_pos={esa_pos}, use_swinv2={use_swinv2}')
        print(f'[DEBUG] ESA count: {esa_count}, GAB count: {gab_count}, Params: {total_params:,}')
